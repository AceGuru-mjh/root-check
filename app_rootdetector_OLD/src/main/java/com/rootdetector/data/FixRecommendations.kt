...
object FixRecommendations {
    ...
    private val recommendations: Map<String, FixRecommendation> = mapOf(
        ...
        "系统调用时序" to FixRecommendation(
            ...
            stepsZh = listOf(
                ...
                "3. 检查系统调用表: 需要内核级访问权限",
                "4. 对比 /proc/kallsyms 中的系统调用地址与官方值",
            ...
            stepsEn = listOf(
                ...
                "3. Check syscall table: requires kernel-level access",
                "4. Compare syscall addresses in /proc/kallsyms with official values",
        ...
        "Shamiko" to FixRecommendation(
            ...
            descriptionZh = "检测到 Shamiko 行为特征。Shamiko 是 Zygisk 模块，用于隐藏 Root 状态，通过动态替换 Zygisk 上下文、覆写 SELinux 策略、挂载过滤和进程白名单等机制绕过检测。",
            descriptionEn = "Detected Shamiko behavior characteristics. Shamiko is a Zygisk module that hides Root status by dynamically replacing Zygisk context, overriding SELinux policy, mount filtering, and process whitelisting.",
            stepsZh = listOf(
        ...
        "ZygiskNext" to FixRecommendation(
            ...
            descriptionZh = "检测到 ZygiskNext 特征。ZygiskNext 是 Zygisk 的替代实现，通过内存隔离标记、匿名命名空间逃逸拦截和私有 memfd 通道加载模块，提供更强的隐蔽性。",
            descriptionEn = "Detected ZygiskNext characteristics. ZygiskNext is an alternative Zygisk implementation using memory isolation marks, anonymous namespace escape interception, and private memfd channels for module loading.",
            stepsZh = listOf(
        ...
        "APatch KPM" to FixRecommendation(
            ...
            descriptionZh = "检测到 APatch KPM（Kernel Patch Module）内核补丁。APatch 通过 KPM 模块在内核层执行补丁操作，truncate 后门检测到异常时序，且 /sys/kpm/ 节点存在。",
            descriptionEn = "Detected APatch KPM (Kernel Patch Module). APatch patches the kernel via KPM modules; truncated backdoor timing anomalies detected and /sys/kpm/ node exists.",
            stepsZh = listOf(
                "1. 检查 /sys/kpm/ 目录是否存在 KPM 模块节点",
                "2. APatch 的 truncate 后门通过特定参数值触发内核代码执行",
                "3. 建议: 使用 APatch 官方卸载工具移除",
                "4. 检查 kallsyms 是否被擦除: cat /proc/kallsyms | grep kallsyms_lookup_name",
            ...
            stepsEn = listOf(
                ...
                "4. Check kallsyms erasure: cat /proc/kallsyms | grep kallsyms_lookup_name",
        ...
        "KernelSU overlay" to FixRecommendation(
            id = "kernelsu_overlay",
            titleZh = "KernelSU overlayfs 伪装检测",
            titleEn = "KernelSU overlayfs Masquerading Detected",
            descriptionZh = "检测到 KernelSU 通过 overlayfs 伪装系统分区。KernelSU 劫持 overlay 挂载点，使用私有挂载参数和镜像文件修改系统分区内容。",
            descriptionEn = "Detected KernelSU overlayfs masquerading on system partitions. KernelSU hijacks overlay mount points using private mount parameters and image files.",
            stepsZh = listOf(
        ...
        "LSPosed" to FixRecommendation(
            ...
            descriptionZh = "检测到 LSPosed（旧版 Riru 或新版 Zygisk 模式）框架。LSPosed 通过 liblspd binder 私有通道通信，可 Hook 系统框架实现高级功能隐藏。",
            descriptionEn = "Detected LSPosed framework (legacy Riru or new Zygisk mode). LSPosed uses liblspd binder private channels for communication and can hook system framework.",
            stepsZh = listOf(
    ...
    fun getRecommendation(layer: String): FixRecommendation? {
        return recommendations.entries.firstOrNull { layer.contains(it.key) }?.value
    }
