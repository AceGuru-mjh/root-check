...
void apex_pmu_spoof_read(void *buf, size_t count) {
    ...
    fake.cycles = clean_pmu.cycles + (random() % 100);
    fake.instructions = clean_pmu.cycles * 2 + (random() % 50);
    fake.cache_refs = clean_pmu.cycles / 3 + (random() % 20);
    fake.cache_misses = clean_pmu.cache_refs / 10 + (random() % 5);
    fake.branches = clean_pmu.instructions / 4 + (random() % 30);
    fake.branch_misses = fake.branches / 20 + (random() % 2);
